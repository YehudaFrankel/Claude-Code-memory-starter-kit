...
Clankbrain doesn't touch your data. Memory stays on your machine. Kit updates flow one way only from Clankbrain to you, never the reverse. No telemetry, no analytics, no servers. The real data consideration for businesses is Claude Code itself sending prompts to Anthropic and that's true regardless of this kit. For regulated industries get an Anthropic enterprise plan with a BAA. Clankbrain adds nothing to that surface. Memory files are local, contain code patterns and decisions not personal data, and are fully under your control.